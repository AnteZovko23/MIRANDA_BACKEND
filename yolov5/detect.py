...
channel.queue_declare(queue='face_detections')
...
#  files() and commands() in parallel
...
predictor = dlib.shape_predictor("../embedder/shape_predictor_68_face_landmarks.dat")

face_rec = frl.Face_Recognition_Library(DEBUG=True)
# try:
#     commands()
...
def run(
        weights=ROOT / 'yolov5m.engine',  # model path or triton URL
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c == 21: # Face
                            ...
                            normalized_face = face_rec.get_normalized_face(im0, rect)
                            # face_box = cv2.cvtColor(im0[p1[1]:p2[1], p1[0]:p2[0]], cv2.COLOR_BGR2RGB)
                            # face_box = np.array(normalized_face)
                            # cv2.imwrite("face_box.jpg", face_box)
                            embeddings = face_rec.get_embeddings(normalized_face=normalized_face)
                            try:
                                label, voice_rec = face_rec.recognize_face(face_embeddings=None, target_embedding=embeddings)
                                if label != "unknown":
                                    channel.basic_publish(exchange='', routing_key='face_detections', body=voice_rec)
                            except Exception as e:
                                print(e)
                                label = "Unknown"
                        
                        # label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                        annotator.box_label(xyxy, label, color=colors(c, True), predictor=predictor, landmarks=False, bbox=True)
                    if save_crop:
